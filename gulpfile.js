...
function packageFirefox() {
    return new Promise((resolve, reject) => {
        _7z.pack(`./${outputFolderFirefox}/*`, `${baseOutputFolder}/youtube-clean-fullscreen_firefox_${version}.zip`, (err) => {
            if (err) {
                reject(err);
            }
            else {
...
function buildChrome() {
    const singleFiles = src(baseSourceFolder + "manifest_chrome.json")
        ...
        .pipe(src(baseSourceFolder + "player.css"))
        .pipe(src(baseSourceFolder + "popup.html"))
        .pipe(src(baseSourceFolder + "popup.css"))
        .pipe(src(baseSourceFolder + "popup.js"));

    const scripts = src([baseSourceFolder + "content-script.js", baseSourceFolder + "player.js"])
        .pipe(rollup({
...
function packageChrome() {
    return new Promise((resolve, reject) => {
        _7z.pack(`./${outputFolderChrome}/*`, `${baseOutputFolder}/youtube-clean-fullscreen_chrome_${version}.zip`, (err) => {
            if (err) {
                reject(err);
            }
            else {
